...
public class PatientDisagreeWithPrescriptionCommand implements Command {
    private static final Logger logger = LoggerFactory.getLogger(
            PatientDisagreeWithPrescriptionCommand.class);
    ...
    public void execute(HttpServletRequest req, HttpServletResponse res)
        ...
        try {
            isDisagree = patientService.disagreeWithPrescription(user,
                    prescriptionId, disagreementDescription);
        } catch (ServiceException e) {
            logger.error("An error while a patient disagrees with a non " +
                    "medicine prescription.", e);
