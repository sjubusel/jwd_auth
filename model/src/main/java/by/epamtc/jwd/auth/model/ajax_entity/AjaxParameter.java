package by.epamtc.jwd.auth.model.ajax_entity;

public final class AjaxParameter {
    public static final String COUNTRY = "countryInput";
    public static final String REGION = "regionInput";
    public static final String AREA = "areaInput";
    public static final String SETTLEMENT = "settlementInput";
    public static final String ROAD = "roadInput";

    public static final String COUNTRY_ID = "hiddenCountryId";
    public static final String REGION_ID = "hiddenRegionId";
    public static final String AREA_ID = "hiddenAreaId";
    public static final String SETTLEMENT_ID = "hiddenSettlementId";

    public static final String RECIPIENT_PERSON = "recipientInput";

    public static final String AJAX_CONTENT_TYPE = "application/json";
    public static final String AJAX_CHARACTER_ENCODING = "UTF-8";
    public static final String FOOD_TYPE = "foodTypeInput";
    public static final String MEDICINE_TYPE = "medicineTypeInput";
    public static final String DISEASE = "diseaseInput";
    public static final String VISIT_PERSON = "personInput";
    public static final String DISEASE_PART = "diseaseInput";
    public static final String MEDICINE_PART = "medicineInput";

    private AjaxParameter() {
    }
}
